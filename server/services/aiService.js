...
const generateWithRetry = async (model, prompt, priority = "high") => {
    ...
    if (quotaStatusZero) {
        throw new Error("ACCOUNT_RESTRICTED: Your Google account has 0 quota for this region. Please try a different API project or region.");
    }
    ...
    if (Date.now() < aiCircuitBreaker && priority === "low") {
        console.warn("🛡️ AI Circuit Breaker active: Skipping background AI request to save quota.");
    ...
    for (let i = 0; i < maxRetries; i++) {
        try {
            ...
            if (errString.includes("quota_limit_value") && errString.includes('"0"')) {
                quotaStatusZero = true;
                console.error("❌ CRITICAL: Google has set your AI quota to ZERO. This key is restricted.");
                throw new Error("ACCOUNT_RESTRICTED");
            }

            // Rate Limit checks
            if (errString.includes("429") || errString.includes("too many requests") || errString.includes("quota exceeded")) {
                aiCircuitBreaker = Date.now() + 65000; // Suspend background tasks for 65 seconds

                if (i < maxRetries - 1) {
                    console.warn(`⚠️ API Rate limit hit (Attempt ${i + 1}/${maxRetries}). Waiting ${delay / 1000}s to reset...`);
...
Text: "${text}"`;
            ...
            console.warn("Moderation fallback (allowed) due to service error:", error.message);
            return { flagged: false, categories: {}, severity: "low", error: error.message };
        }
    },
