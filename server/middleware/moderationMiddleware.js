    ...
    try {
        ...
        if (moderationResult.flagged) {
            ...
            await ModerationLog.create({
                userId: req.user._id,
                messageText: text,
                moderationScore: moderationResult.severity === "high" ? 0.9 : moderationResult.severity === "medium" ? 0.6 : 0.3,
            ...
            if (user.moderationViolations >= 5 && !user.isBanned) {
                ...
                return res.status(403).json({
                    success: false,
                    message: "Your account has been banned due to repeated violations of community guidelines",
                    banned: true,
                });
            }
...
export const checkBanStatus = async (req, res, next) => {
    try {
        ...
        if (user.isBanned) {
            return res.status(403).json({
                success: false,
                message: "Your account has been banned due to violations of community guidelines",
                banned: true,
            });
        }
